...
async def cmd_start(message: Message) -> None:
    ...
    await message.answer(
        ...
        "📎 <b>Требования:</b>\n"
        "• Подпись-описание (обязательно в поле подписи к медиа)\n"
        "• Ровно <b>2 медиафайла</b> одним альбомом\n"
        "• Можно комбинировать: 2 фото, 2 видео или 1 фото + 1 видео\n\n"
...
async def handle_media_group(message: Message) -> None:
    ...
    try:
        ...
        existing_ids = [m["file_id"] for m in group_data["media"]]
    ...
    if not completed_data["caption"]:
        ...
        await message.answer(
            ...
            "Отправь альбом снова и добавь текст-описание в поле подписи."
        )
        return
...
async def handle_single_media(message: Message) -> None:
    ...
    if state == "waiting_content":
        await message.answer(
            "❌ Нужно отправить ровно <b>2 медиафайла</b> <u>одним альбомом</u>.\n"
            "Можно комбинировать фото и видео. Выдели оба файла и отправь вместе."
        )
...
async def handle_plain_text(message: Message) -> None:
    ...
    if state == "waiting_content":
        await message.answer(
            ...
            "Прикрепи <b>2 медиафайла</b> альбомом и добавь текст в подпись."
        )


async def _build_media_list(media: list[dict], caption: str) -> list:
    result = []
    for i, item in enumerate(media[:2]):
